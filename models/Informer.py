...
from layers.SelfAttention_Family import ProbAttention, AttentionLayer, VarCorAttentionLayer, VarCorAttention
...
class Model(nn.Module):
    ...
    def long_forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if True:
            dec_out = self.decoder(
                dec_out, enc_out, x_mask=None, cross_mask=None).permute(0, 2, 1)
        else:
            dec_out = self.decoder(
    ...
    def short_forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        std_enc = torch.sqrt(torch.var(
            x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()  # B x 1 x E
        x_enc = x_enc / std_enc
