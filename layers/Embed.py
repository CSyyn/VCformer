...
class DSEmbedding(nn.Module):
    """
       Embedding from DSformer
    """
    ...
    def __init__(self, Input_len, num_id, num_samp, IF_node):
        ...
        """
        super(DSEmbedding, self).__init__()
        self.IF_node = IF_node
        self.num_samp = num_samp  # number of patches
    ...
    def forward(self, x):
        ...
        node_emb1 = self.node_emb.unsqueeze(0).expand(
            batch_size, -1, -1).unsqueeze(-1)

        x_1 = DSEmbedding.down_sampling(x, self.num_samp)
        if self.IF_node:
            x_1 = torch.cat([x_1, DSEmbedding.down_sampling(
                node_emb1, self.num_samp)], dim=-1)

        x_2 = DSEmbedding.Interval_sample(x, self.num_samp)
        if self.IF_node:
            x_2 = torch.cat([x_2, DSEmbedding.Interval_sample(
                node_emb1, self.num_samp)], dim=-1)
        return x_1,x_2
    @staticmethod
